...
const breeds = ['Holstein', 'Suizo Pardo', 'Jersey', 'Brahman', 'Angus', 'Hereford', 'Charolais', 'Simmental', 'Criollo', 'Limousin', 'Otro']
...
function RegistrarAnimal() {
  ...
  const canSubmit = useMemo(() => {
    const hasBaseFields = form.arete.trim() && form.especie && form.sexo && form.fecha && Number(form.peso) > 0
    const hasPrice = form.origen === 'nacio' || Number(form.precio_compra) > 0
    return Boolean(hasBaseFields && hasPrice && status !== 'loading')
  }, [form, status])
  ...
  function selectOrigin(origen) {
    setForm((current) => ({
      ...current,
      origen,
      precio_compra: origen === 'nacio' ? '0' : current.precio_compra === '0' ? '' : current.precio_compra,
    }))
  }
  ...
  async function handleImage(event) {
    ...
    setPreview(dataUrl)
    setForm((current) => ({ ...current, foto_path: dataUrl }))
  }
      ...
      <form className="mx-auto max-w-4xl space-y-8 px-4 py-8" onSubmit={handleSubmit}>
        ...
        <section className="rounded-2xl bg-white p-6 shadow-[0_10px_30px_rgba(29,29,27,0.07)]">
          ...
          <div className="mt-5 flex flex-wrap gap-4">
            {breeds.map((breed) => {
              const active = form.especie === breed
              return (
                <button
                  ...
                  key={breed}
                  onClick={() => setForm((current) => ({ ...current, especie: breed }))}
                  type="button"
                >
                  {breed}
                </button>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="mt-5 grid gap-4">
            <button className={`min-h-12 rounded-2xl border px-3 text-sm font-bold ${form.origen === 'nacio' ? 'border-[#07612d] bg-[#07612d] text-white shadow-[0_8px_18px_rgba(7,97,45,0.18)]' : 'border-[#98a287]/25 bg-white hover:bg-[#F4F4F4]'}`} onClick={() => selectOrigin('nacio')} type="button">
              Nació en el rancho ($0)
            </button>
            <button className={`min-h-12 rounded-2xl border px-3 text-sm font-bold ${form.origen === 'comprado' ? 'border-[#07612d] bg-[#07612d] text-white shadow-[0_8px_18px_rgba(7,97,45,0.18)]' : 'border-[#98a287]/25 bg-white hover:bg-[#F4F4F4]'}`} onClick={() => selectOrigin('comprado')} type="button">
              Comprado
            </button>
          </div>
        ...
        <section className="rounded-2xl bg-white p-6 shadow-[0_10px_30px_rgba(29,29,27,0.07)]">
          ...
          <div className="mt-6 flex min-h-80 items-center justify-center overflow-hidden rounded-2xl border border-[#98a287]/18 bg-[#F4F4F4] px-4 text-center text-base font-semibold text-[#98a287]">
            {preview ? <img alt="Vista previa del animal" className="h-full max-h-80 w-full object-cover" src={preview} /> : 'Sin fotografía capturada'}
          </div>
        </section>

        {message ? <div className="rounded-2xl border border-[#D32F2F]/20 bg-[#D32F2F]/10 p-4 text-sm font-semibold text-[#D32F2F]">{message}</div> : null}

        <button className="min-h-16 w-full rounded-2xl bg-[#07612d] px-5 text-lg font-bold text-white shadow-[0_10px_22px_rgba(7,97,45,0.2)] disabled:cursor-not-allowed disabled:bg-[#98a287]" disabled={!canSubmit} type="submit">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-[#e6e2d7] bg-white px-2 py-2 shadow-[0_-8px_24px_rgba(29,29,27,0.06)] md:hidden">
        <div className="mx-auto grid max-w-xl grid-cols-5">
          {bottomNav.map(({ label, to, icon: Icon }) => (
            <NavLink className={({ isActive }) => `flex min-h-14 flex-col items-center justify-center gap-1 text-xs font-semibold ${isActive ? 'text-[#07612d]' : 'text-[#98a287]'}`} key={label} to={to}>
              <Icon size={21} />
              {label}
            </NavLink>
          ))}
