import { obtenerTotalesCategoria, obtenerEntradaPrincipal, mxn } from './expenseUtils';

function ResumenGasto({ expenses: gastos }) {
  const categoryTotals = obtenerTotalesCategoria(gastos);
  const topCategory = obtenerEntradaPrincipal(Object.entries(categoryTotals));
  const total = gastos.reduce((suma, gasto) => suma + Number(gasto.precio), 0);
  ...
  return (
    <section className="rounded-2xl border border-[#98a287]/18 bg-white p-4 shadow-[0_12px_28px_rgba(29,29,27,0.07)] md:p-5">
      ...
      <div className="mt-5 grid gap-3">
        {Object.entries(categoryTotals).map(([category, valor]) => {
