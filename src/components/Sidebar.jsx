...
function Sidebar({ isOpen, onClose }) {
  return (
    <>
      {isOpen ? <button aria-label="Cerrar navegación" className="fixed inset-0 z-30 bg-[#1d1d1b]/30 lg:hidden" onClick={onClose} type="button" /> : null}
      <aside
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button aria-label="Cerrar menú" className="rounded-xl p-2 text-[#07612d] lg:hidden" onClick={onClose} type="button">
            <X size={22} />
          </button>
        </div>
